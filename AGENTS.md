...
| `PLANNOTATOR_OTTY` | Set to `1` / `true` / `yes` to open the session in Otty (`otty view <url>`) instead of the system browser. Same as `--otty`. Extra `otty view` flags: `--otty -- <args>`, `--otty-args "<args>"`, or `PLANNOTATOR_OTTY_ARGS`. The CLI still waits for the reviewer. Cannot be combined with `--browser`. |
...
The default code-review diff is **`since-base`** — a composite of `merge-base(base, HEAD)` vs the working tree plus untracked files ("everything a PR would show if you committed and pushed now"). It can render as a three-section **git status** panel (Committed / Changes / Untracked) via `SectionsPanel`, with a `Tree | Git status | Commits` toggle (`PanelViewToggle`). The Commits segment (git-local sessions only) is a gitlane-style commit graph (`CommitsPanel`): coloured lanes, ref chips (heads / remotes / tags / stashes), an uncommitted node when the worktree is dirty, and a right-click menu for git actions (checkout, branch, merge, rebase, reset, stash, push/fetch/pull). Clicking a commit still opens its own diff (`commit:<sha>`, vs its first parent) as the all-files view headed by the commit message rendered as markdown — details stay in the center dock, not an inline expander. Clicking the uncommitted node restores the review the session came from. The Commits view is a self-contained detour: entering it memoizes the previously active diff, exiting to Tree restores that diff verbatim (exiting to Git status resets to `since-base` as always), the memo clears whenever any non-commit diff is applied, and a reload that serves a commit-family diff with a non-Commits panel view snaps once to the session default so the commit diff cannot outlive the visit. The toggle never writes the persisted `reviewPanelView` / `defaultDiffType` preferences (no server writes from a toggle click), but it does record a cookie-only last-used memo (`reviewPanelViewLastUsed`, `sections` | `tree` — never `commits`; the Commits view is session-only). A review OPENS on session choice ?? last-used memo ?? persisted `reviewPanelView` (cookie-only, written only by Settings and `ReviewSetupDialog` through `setReviewPanelView()`, which also syncs the memo so an explicit choice is never shadowed by a stale one). The first-run initializer marks review-setup-seen when it seeds the cookie-only Tree choice, not only on dismiss, so it is genuinely one-time per browser and cannot overwrite a returning reviewer's persisted or last-used view; it inherits the resolved `defaultDiffType` without a server config write. Persistence of `reviewPanelView` and `defaultDiffType` is independent — Settings / ReviewSetup may store any combination. Runtime still gates Git status: `SectionsPanel` only mounts while the live diff is `since-base` (`sectionsAvailable`); otherwise `resolvePanelView` falls back to Tree for that session without rewriting either preference. The header Git status toggle switches the live diff to `since-base` for the session (SectionsPanel cannot render anything else) and does not change Default Diff. Entering Git status from Tree while a classic diff is live memos that diff; leaving Git status for Tree restores it. A picker change away from since-base drops the memo so Tree cannot overwrite the new choice.
...
| `/api/commits`        | GET    | Commit graph for the Commits panel (`?limit=&remotes=&stashes=&tags=&branch=`) → `{ commits, layout, hasMore, base, head, branch, detached, remotes, branches }`. Commits carry parents, ref chips (`heads` / `tags` / `remotes` / `stash`), `isHead`, and best-effort author `avatarUrl`; `layout` is the lane/polyline model the client paints. `limit` is a newest-first window (not a `--first-parent` cursor). Plain local git sessions only (PR/workspace/GitButler/jj/p4 → 400); computed against the active diff's cwd, so worktree sessions list the worktree's history. |
| `/api/git-graph/action` | POST | Git write action from the commit-graph context menu (`{ action, params }`). Same git-local gate as `/api/commits`. |
| `/api/git-graph/tag`  | GET    | Annotated-tag details for the graph's "View details" menu (`?name=`). Same git-local gate as `/api/commits`. |
