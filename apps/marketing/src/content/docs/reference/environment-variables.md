...
| `PLANNOTATOR_OTTY` | unset | Set to `1` / `true` / `yes` to open the session in Otty (`otty view <url>`) instead of the system browser. Same as `--otty`. Extra flags: `--otty -- <args>`, `--otty-args`, or `PLANNOTATOR_OTTY_ARGS`. The process still waits for the reviewer. Cannot be combined with `--browser`. |
