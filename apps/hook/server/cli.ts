...
export function formatTopLevelHelp(): string {
  return [
    ...
    "  plannotator [--browser <name>] [--otty [-- <otty-args>]]",
    "  plannotator review [--git | --gitbutler] [--tailscale] [--otty] [PR_URL]",
    "  plannotator annotate <file.md | file.txt | file.html | https://... | folder/>  [--markdown] [--no-jina] [--tailscale] [--otty] [--gate] [--json] [--hook] [--require-approval] [--result-file <path>]",
...
export const SUBCOMMAND_HELP: Record<string, string> = {
  review: [
    ...
    "  --otty        Open in Otty (`otty view <url>`) instead of the system browser",
    "  --otty-args <args>",
    "                Extra flags forwarded to `otty view` (implies --otty)",
    "  --            After --otty, remaining tokens are forwarded to Otty",
